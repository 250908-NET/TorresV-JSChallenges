...
While many of these are simple challenges, please remember that part of our training to being able to prove what you know and what you learned along the way. We want to keep building good habits of tracking our work through Git. We'll be continuing to build out the repo that you started earlier for the HTML challenges. As you complete each Task, please create a branch with the task name/number (task_8), push the branch to your repo, then merge it into the main branch of your repo. You should NOT be committing directly to the main branch.
...
Mini Challenge
Write a function called square() that takes a number and returns its square. Example: square(4) → 16
...
Press F12 → open the Console to see the output.
...
Mini Challenge
Add another button that runs a new function when clicked. Example:
...
Open index.html in your browser and check the Console. You should see the message logged from script.js.
Tip: Always include your <script> tag just before </body>, so the HTML loads first before the JS runs. Yes, there are times where we want the script to run first, but not for what we're doing here.
...
Mini Challenge
Add a second input field for the user’s favorite color. When the button is clicked, greet the user and mention their color in the message.
...
MDN – Adding JavaScript to a Web Page
JavaScript.info – The Script Tag
W3Schools – JS HTML DOM Tutorial
...
Create a page with a heading and a button. When the button is clicked, change the heading text to “You clicked the button!”
...
Create a list (<ul>). Add a button that appends a new <li> with the text “New Item” each time it’s clicked.
...
Place an image and a button on the page. Each time the button is clicked, toggle the image between two different sources (e.g., cat.jpg ↔ dog.jpg).
...
Create a button that changes the background color of the page each time it’s clicked. Use document.body.style.backgroundColor.
...
Arrow functions do not have their own this. Instead, they inherit this from their surrounding scope (called lexical scoping).
...
✅ Goal: Write it using the shortest valid arrow function form.
...
🧭 Tip: Arrow functions are great for short, focused logic — but don’t overuse them. Use regular functions when you need clear structure or access to this.
...
Create an HTML page with a <div id="output"></div>.
Fetch a post from https://jsonplaceholder.typicode.com/posts/1.
Display the post’s title and body inside the output div.
Hint:
...
Use the endpoint https://jsonplaceholder.typicode.com/posts.
Use slice(0, 5) to select the first five posts.
Create HTML for each post and append them to the page dynamically.
Hint:
...
Add a <button id="loadBtn">Load Posts</button> to your HTML.
Add an event listener that runs your fetch code when clicked.
Hint:
...
Add an <input id="postId" type="number" /> and a <button id="fetchBtn">Fetch Post</button>.
When clicked, get the value of postId.
Fetch from https://jsonplaceholder.typicode.com/posts/{id}.
Display the result.
Hint:
...
Create a button that loads a random post (use Math.random()).
Display user data from https://jsonplaceholder.typicode.com/users.
Make a small “load more” feature that fetches and adds new posts to the list each time you click a button.
**Recommended Reading **
...
👉 https://www.freecodecamp.org/news/how-to-use-the-javascript-fetch-api-to-get-data/
